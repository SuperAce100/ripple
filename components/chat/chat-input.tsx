...
export default function ChatInput({ handleSubmit, input, handleInputChange, placeholder }: { handleSubmit: (e: React.FormEvent<HTMLFormElement>) => void, input: string, handleInputChange: (e: React.ChangeEvent<HTMLInputElement>) => void, placeholder: string }) {
  return (
    <form onSubmit={handleSubmit}  className="flex flex-row items-center justify-center rounded-3xl w-full relative bg-background">
        <Input name="prompt" value={input} onChange={handleInputChange} className="w-full text-lg p-6 py-8 rounded-3xl shadow-2xl shadow-primary/20 delay-75" placeholder={placeholder} autoComplete="off" />
